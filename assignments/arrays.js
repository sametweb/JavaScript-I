...
let requestedCar = inventory.filter(item => item.id === 33)[0];

console.log(
  `Car 33 is a ${requestedCar.car_year} ${requestedCar.car_make} ${requestedCar.car_model}`
);
...
let carModels = [];
let carModelsSorted = [];
carModels = inventory.map(item => item.car_model);
console.log("Car models alphabetically listed", carModels.sort());

// ==== Challenge 4 ====
// The accounting team needs all the years from every car on the lot. Create a new array from the dealer data containing only the car years and log the result in the console.
let carYears = [];
carYears = inventory.map(item => item.car_year);
console.log("All the years from every car", carYears);

// ==== Challenge 5 ====
// The car lot manager needs to find out how many cars are older than the year 2000. Using the carYears array you just created, find out how many cars were made before the year 2000 by populating the array oldCars and logging it's length.
let oldCars = [];
oldCars = carYears.filter(item => item < 2000);
console.log("Numbers of cars that are made before 2000 is", oldCars.length);

// ==== Challenge 6 ====
// A buyer is interested in seeing only BMW and Audi cars within the inventory. Return an array that only contains BMW and Audi cars.  Once you have populated the BMWAndAudi array, use JSON.stringify() to show the results of the array in the console.
let BMWAndAudi = [];
inventory
  .filter(item => item.car_make === "BMW" || item.car_make === "Audi")
  .map(item => {
    BMWAndAudi.push(item);
  });
console.log("BMW and Audi cars in the inventory", JSON.stringify(BMWAndAudi));
